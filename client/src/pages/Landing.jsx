...
const MINI_PLANS = [
  { icon: '🧘‍♀️', name: 'Yoga',        status: 'ready' },
  { icon: '🏋️',   name: 'Fitness',     status: 'ready' },
  { icon: '🥗',   name: 'Nutrition',   status: 'ready' },
  { icon: '🌿',   name: 'Panchakarma', status: 'ready' },
  { icon: '🍵',   name: 'Remedies',    status: 'generating' },
  { icon: '💊',   name: 'Medicines',   status: 'queued' },
]
      ...
      <div className="lnd-prev-plans fmv-plans-grid">
        {MINI_PLANS.map(p => (
          <div key={p.name} className={`lnd-prev-plan${p.status === 'generating' ? ' fmv-generating' : ''}`}>
            <div className="lnd-prev-plan-icon">{p.icon}</div>
...
function SafetyFeatureVisual() {
  return (
    <div className="lnd-app-preview fmv-fill">
      ...
      <div className="fmv-all-clear-badge">🛡️ 0 contraindications detected</div>
    </div>
  )
}
