...
- ☐ **The gym library was rebuilt on 2026-08-19 and is entirely unreviewed.** The
  previous entry here described 902 imported exercises with 884 rule-generated safety
  tags. That library is gone: it came from a script that re-derived `category`, `level`
  and `mechanic` from substring matches on the exercise name, and 176 of 779 rows — 23% —
  contradicted their own upstream source.
  - **173 curated movements**, every field authored: programming, instructions, coaching
    cues, and a clinical judgement per movement with the mechanism stated in a sentence.
  - **0 of 173 carry `contraindications_reviewed`**, down from 18. That is a real cost and
    not a regression to argue with — the 18 were foam-rolling entries and specialty
    variants (`Brachialis-Smr`, `Gironda Sternum Chins`, `London Bridges`) that a curated
    library correctly does not contain. No review of a surviving movement was discarded.
  - The review is `server/data/golden/vaidya_gym_packet.md` — now about **half a day**.
    The reviewer reads 173 stated mechanisms and agrees or disagrees, rather than auditing
    keyword matches. `gym_exercise_review.csv` shows each judgement beside
    `what_a_rule_would_have_said`; **they differ on 89 of the 173**, and that column is
    where to start.
  - ☐ **Four engineering decisions need a clinician to confirm or reverse** (packet, "Four
    decisions"): prenatal suitability as a per-movement judgement with 91 of 173 passing;
    `hypertension` as the age proxy for cardiovascular strain at 60+; impact work withheld
    at obese BMI and whether it should extend to `overweight`; and `skill_floor` split from
    `level` — whether a movement can be *performed* versus whether it should be
    *programmed*.
  - What engineering *can* certify is the programming, not the judgements: a plan sweep
    across eleven profile types confirms no movement carrying a contraindication reaches a
    practitioner who declared that condition, and that landing impact reaches none of the
    four groups it is withheld from. That is gating working, not clinical correctness.
  - ⚠️ **Eleven defects were found in the gym feature on 2026-08-19**, on a feature that had
    already shipped across 34 merged PRs. Nine share one shape: the safety model stopped at
    the boundary of the structured data. Warm-ups, cool-downs, rest-day activities and
    Ayurvedic tips were prose, and prose passes no gate — a 70-year-old with hypertension, a
    118 kg practitioner with a knee replacement and a pregnant practitioner each received a
    main workout with every impact movement filtered out, and then opened the session with
    "Jumping jacks — 30 sec". `Kapalabhati` was prescribed by name to hypertensive, cardiac
    and pregnant users in two separate fields, while `yoga_plan_engine` has gated exactly
    that for months. All are fixed and covered. **The rate of discovery has not reached
    zero**, which is the honest state of this feature rather than a claim of completeness.
  - Rebuild and regenerate after any change:
    `python scripts/build_gym_library.py && python scripts/build_gym_review_packet.py`
    (the library is generated from `scripts/gym_library/`; `--check` and a test enforce it).
...
disease→dosha mappings, the pregnancy gating, the 113 rewritten pose entries, or any of the
173 curated gym movements. The medical disclaimer in the Terms mitigates but does not
